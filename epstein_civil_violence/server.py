...
class EpsteinCivilViolenceServer(EpsteinCivilViolence_RL):
    ...
    def __init__(self, height=20, width=20, citizen_density=0.5, cop_density=0.1, citizen_vision=4, cop_vision=4, legitimacy=0.82, max_jail_term=30, model_path=None):
        super().__init__(height, width, citizen_density, cop_density, citizen_vision, cop_vision, legitimacy, max_jail_term)
        self.running = True
        self.iteration = 0
        def env_creator(_):
            return EpsteinCivilViolence_RL(height, width, citizen_density, cop_density, citizen_vision, cop_vision, legitimacy, max_jail_term)
    ...
    def step(self):
        ...
        for agent in self.schedule.agents:
            if agent.unique_id.startswith("cop"):
                action_dict[agent.unique_id] = self.cop_policy.compute_single_action(np.array(observation[agent.unique_id]).T, explore=False)[0]
            else:
                action_dict[agent.unique_id] = self.citizen_policy.compute_single_action(np.array(observation[agent.unique_id]).T, explore=False)[0]
...
model_params = {
    ...
    "width": 20,
    "model_path": None,
    "citizen_density": mesa.visualization.Slider("Initial Agent Density", 0.5, 0.0, 0.9, 0.1),
    "cop_density": mesa.visualization.Slider("Initial Cop Density", 0.1, 0.0, 0.3, 0.01),
    "citizen_vision": mesa.visualization.Slider("Citizen Vision", 4, 1, 10, 1),
    "cop_vision": mesa.visualization.Slider("Cop Vision", 4, 1, 10, 1),
    "legitimacy": mesa.visualization.Slider("Government Legitimacy", 0.82, 0.0, 1, 0.01),
