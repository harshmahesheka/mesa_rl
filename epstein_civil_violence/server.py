...
class EpsteinCivilViolenceServer(EpsteinCivilViolence_RL):
    ...
    def __init__(self, height=5, width=5, citizen_density=0.5, cop_density=0.05, citizen_vision=4, cop_vision=4, legitimacy=0.82, max_jail_term=30, model_path=None):
    ...
    def step(self):
        ...
        for agent in self.schedule.agents:
            if agent.unique_id.startswith("cop"):
                action_dict[agent.unique_id] = self.cop_policy.compute_single_action(np.array(self.observation[agent.unique_id]).T, explore=False)[0]
            else:
                action_dict[agent.unique_id] = self.citizen_policy.compute_single_action(np.array(self.observation[agent.unique_id]).T, explore=False)[0]
...
model_params = {
    ...
    "cop_density": mesa.visualization.Slider("Initial Cop Density", 0.05, 0.0, 0.3, 0.01),
    ...
    "max_jail_term": mesa.visualization.Slider("Max Jail Term", 5, 0, 50, 1),
}
